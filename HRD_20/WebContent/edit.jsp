<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="Shop.*" %>
<%int number=Integer.parseInt(request.getParameter("originno")); %>
<%ShopDTO dto=ShopDAO.getUser(number); %>
...
<script type="text/javascript" src="views/script.js"></script>
	<section>
		...
		<form action="/edit" name="regForm" method="post">
		<table border="1">
			<input type="hidden" name="originno"value=<%=request.getParameter("originno") %>>
			<tr>
				<td>회원번호(자동발생)</td>
				<td><input type="text" name="custno" value="<%=request.getParameter("originno")%>"></td>
			</tr>
			<tr>
				<td>회원성명</td>
